/*
===================================================================================
Script    : 01_etl_run_bronze_pipeline
...
===================================================================================
Script Purpose:
    It loads the bronze layer with data from the source system. 
...
Usage: EXEC etl.run_bronze_pipeline
...
CREATE OR ALTER PROCEDURE etl.run_bronze_pipeline AS
BEGIN
	...
	DECLARE
	@batch_id INT,
	@batch_name NVARCHAR(50) = 'etl.run_bronze_pipeline',
	...
	-- =======================================================================================
	BEGIN TRY
		EXEC etl.load_bronze_crm;
		EXEC etl.load_bronze_hrms;
		EXEC etl.load_bronze_cbs;
		EXEC etl.load_bronze_los;

		-- Retrieve total rows processed
		SELECT @total_rows = SUM(COALESCE(total_rows_processed, 0)) FROM etl.batch_log WHERE 
		(batch_name IN ('etl.load_bronze_crm', 'etl.load_bronze_hrms', 'etl.load_bronze_cbs', 'etl.load_bronze_los') 

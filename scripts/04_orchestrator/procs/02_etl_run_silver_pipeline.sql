/*
===================================================================================
Script    : 02_etl_run_silver_pipeline
...
===================================================================================
Script Purpose:
    It loads the silver layer with data from the bronze layer. 
...
Usage: EXEC etl.run_silver_pipeline
...
CREATE OR ALTER PROCEDURE etl.run_silver_pipeline AS
BEGIN
	...
	DECLARE
	@batch_id INT,
	@batch_name NVARCHAR(50) = 'etl.run_silver_pipeline',
	...
	-- =======================================================================================
	-- SECTION 3: LOAD ALL SILVER TABLES
	-- =======================================================================================
	BEGIN TRY
		EXEC etl.load_silver_crm;
		EXEC etl.load_silver_hrms;
		EXEC etl.load_silver_cbs;
		EXEC etl.load_silver_los;

		-- Retrieve total rows processed
		SELECT @total_rows = SUM(COALESCE(total_rows_processed, 0)) FROM etl.batch_log WHERE 
		batch_name IN ('etl.load_silver_crm', 'etl.load_silver_hrms', 'etl.load_silver_cbs', 'etl.load_silver_los') AND
	...
	BEGIN CATCH
		...
		-- Retrieve total rows processed
		SELECT @total_rows = SUM(COALESCE(total_rows_processed, 0)) FROM etl.batch_log WHERE 
		(batch_name IN ('etl.load_silver_crm', 'etl.load_silver_hrms', 'etl.load_silver_cbs', 'etl.load_silver_los') 
